<# 
    ...
    Removes a user by looking up their email address and add them to the given group.
#>
...
try {
    ...
    Write-Output "Removing user $($User.SamAccountName) from group $($ADGroup.Name)"
    Remove-ADGroupMember -Identity $ADGroup -Members $User -Credential $Credential -Confirm:$False
