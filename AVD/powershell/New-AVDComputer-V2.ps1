...
Param(
    ...
    [Parameter(Mandatory=$false)][string]$PowerUser,
    [Parameter(Mandatory=$false)][ValidateLength(4,12)][string]$ComputerPrefix = "avpe-$($Environment.Substring(0,1))",
    ...
    [Parameter(Mandatory=$false)][string]$HostPoolName= "vdpool-$Environment-personal-$LocationName",
    [Parameter(Mandatory=$false)][string]$SubnetName= "internal-desktops-02",
    [Parameter(Mandatory=$false)][string]$TimeZone = "Eastern Time -5:00",
    [Parameter(Mandatory=$false)][string]$Domain = "DOMAIN.net"
)
...
function Set-AZImage {
    ...
    .DESCRIPTION
    Optional parameters are validated for the  Image Definition and Version
    ...
    param (
        ...
        [Parameter(Mandatory=$false)][ValidateSet("gal_avd_nonprod","gal_avd_prod")]
            [string] $GalleryName,
        [Parameter(Mandatory=$false)][ValidateSet("img-avd-nonprod-dev","img-avd-prod-dev")]
            [string] $ImageName,
...
function Remove-FailedHost {
    ...
    $avdHostName = "$ComputerName.$domain"
...
#>
<##!!Connect with AZ Automation!!#>
#Manually Connect to Azure and AzureAD
$User = "matthew.franklin@domain.com"
Connect-AzAccount -AccountId $User | Out-Null
#Connect-AzureAD -AccountId $User

#Azure Subscription ID
$SubscriptionID = (Get-AzSubscription -SubscriptionName "$($Environment)-VDI" | Select-Object Id)
Select-AzSubscription -SubscriptionID $SubscriptionID.Id | Out-Null
#>

#Region Variables
$ComputerPrefix = $ComputerPrefix.ToUpper()
$ouPath= "OU=Personal,OU=$($Environment),OU=AVDOU=Workstations,DC=DOMAIN,DC=net"
...
if($Environment -eq "nonprod"){$TaskSequenceID= "P01229EF"}else{$TaskSequenceID= "P01228FF"}
$WebHookURL = ""
if($PowerUser -like "yes"){$VMSize = "Standard_D2s_v5"}else{$VMSize = "Standard_B2s"}
switch ($TimeZone) {
...
<# Creds stored in Gitlab CICD Vars #>
...
if(!($Storage)){Write-Error "No Azure Storage Account found";break}
...
if(!($StorageAccountKey)){Write-Error "Missing AZ Storage Account Key";break}
#>
#EndRegion Variables
...
if (@($AvailHosts1)){ 
    ...
    if(@($AvailHosts)){
        ...
        if (@($updateUser.AssignedUser -eq $UserUPN)){
            ...
            $UpdateVM = (Get-AzVM -Name $($SessionHostName.replace('.$domain','')) -ResourceGroupName $ResourceGroupName)
...
$Range = 01..1000
$SessionHosts = (Get-AzWvdSessionHost -HostPoolName $HostPoolName -ResourceGroupName $ResourceGroupName | ForEach-Object {($_.Name -split '/')[1] -replace '.$domain',''})
...
$Template =".\New-AVDComputer\Azure-TemplateSpec.json"  
...
Write-Output "Assigning $ComputerName.$domain to $UserUPN in $HostPoolName"
Update-AzWvdSessionHost -HostPoolName $HostPoolName -ResourceGroupName $ResourceGroupName -Name "$ComputerName.$domain" -AssignedUser "$UserUPN"
...
#Region Scratch 
<#
## Test Computer Name against Domain and Azure
function Test-ComputerName {
    ...
    try {
        ...
        Write-Error "Computer Object already exists in $Domain";break
    }
    catch {Write-Output "No Computer Object found in domain '$($Domain)'"}
